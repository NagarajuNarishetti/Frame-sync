  ...
  try {
    ...
    if (invite.keycloak_org_id && invitedUserKeycloakId) {
      try {
        ...
        await axios.post(
          `${process.env.KEYCLOAK_SERVER_URL || 'http://localhost:8080'}/admin/realms/${process.env.KEYCLOAK_REALM || 'framesync'}/organizations/${invite.keycloak_org_id}/members/invite-existing-user`,
          formData,
          {
            headers: {
        ...
        if (statusCode === 500 && errorMessage && errorMessage.toLowerCase().includes('invite email')) {
          try {
            const accessToken = await getKeycloakAdminToken();
            await axios.post(
              `${process.env.KEYCLOAK_SERVER_URL || 'http://localhost:8080'}/admin/realms/${process.env.KEYCLOAK_REALM || 'framesync'}/organizations/${invite.keycloak_org_id}/members`,
