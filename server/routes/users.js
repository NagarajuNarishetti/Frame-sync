...
const createClientRole = async (accessToken, clientUuid, roleName) => {
  try {
    const response = await axios.post(
              `${process.env.KEYCLOAK_SERVER_URL || "http://localhost:8080"}/admin/realms/${process.env.KEYCLOAK_REALM || "framesync"}/clients/${clientUuid}/roles`,
      {
        name: roleName,
...
const getClientRoleByName = async (accessToken, clientUuid, roleName) => {
  try {
    const response = await axios.get(
              `${process.env.KEYCLOAK_SERVER_URL || "http://localhost:8080"}/admin/realms/${process.env.KEYCLOAK_REALM || "framesync"}/clients/${clientUuid}/roles/${encodeURIComponent(roleName)}`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
...
const assignClientRoleToUser = async (
  ...
  try {
    ...
    const response = await axios.post(
              `${process.env.KEYCLOAK_SERVER_URL || "http://localhost:8080"}/admin/realms/${process.env.KEYCLOAK_REALM || "framesync"}/users/${keycloakUserId}/role-mappings/clients/${clientUuid}`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
...
const createKeycloakClient = async (accessToken, username) => {
  try {
    ...
    const response = await axios.post(
              `${process.env.KEYCLOAK_SERVER_URL || "http://localhost:8080"}/admin/realms/${process.env.KEYCLOAK_REALM || "framesync"}/clients`,
      clientPayload,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
...
const checkUserExists = async (accessToken, keycloakId) => {
  try {
    await axios.get(
              `${process.env.KEYCLOAK_SERVER_URL || "http://localhost:8080"}/admin/realms/${process.env.KEYCLOAK_REALM || "framesync"}/users/${keycloakId}`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
  ...
  try {
    ...
    try {
      ...
      const orgResponse = await axios.post(
        `${process.env.KEYCLOAK_SERVER_URL || "http://localhost:8080"}/admin/realms/${process.env.KEYCLOAK_REALM || "framesync"}/organizations`,
      ...
      if (orgResponse.status === 201) {
        ...
        if (!userExists) {
          ...
          await axios.post(
            `${process.env.KEYCLOAK_SERVER_URL || "http://localhost:8080"}/admin/realms/${process.env.KEYCLOAK_REALM || "framesync"}/organizations/${kcOrgId}/members`,
