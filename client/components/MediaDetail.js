      ...
      <div className="relative max-w-full max-h-[70vh]">
        ...
        <div className="absolute inset-0 pointer-events-none">
          {annotations.map((ann) => (
            ...
            >
              <div className="w-4 h-4 bg-blue-500 border-2 border-white rounded-full shadow-2xl group-hover:scale-150 transition-transform"></div>
              <div className="absolute top-6 left-1/2 transform -translate-x-1/2 bg-white backdrop-blur-xl text-gray-800 px-4 py-2 rounded-xl text-xs max-w-[200px] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity border border-blue-200/50 shadow-2xl">
                <div className="font-semibold">{ann.username}</div>
                <div>{ann.text}</div>
                {ann.timestamp !== undefined && (
                  <div className="text-gray-500 text-xs mt-1">{formatTime(ann.timestamp)}</div>
                )}
              </div>
            </div>
          ))}
    ...
    <div className="flex border-b border-white/10">
      <button
        ...
        className={`relative flex-1 py-4 px-4 font-bold text-sm transition-all tracking-wider group ${
          activeTab === 'comments' ? 'text-white' : 'text-gray-300 hover:text-white hover:bg-white/[0.02]'
        }`}
      >
